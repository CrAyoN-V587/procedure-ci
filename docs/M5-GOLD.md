...
- 原冻结快照：`e535370`；冻结后只做执行顺序事实勘误和完成状态注记，PAY-05 gold 行未改
- 阶段：M5b fast-fail
...
| --- | --- | --- | --- | --- | --- | --- |
| PCH-01–06 | B | 3.0.0 | 1.0.1 | 不接受 OpenAPI 3.0.0 | 只改 Arazzo 版本标记后仍被 OpenAPI 3.0.0 阻塞 | 否 |
...
`unknown` 或“发现问题”。本轮基线已确认上述阻塞与邻接工具的覆盖边界，因此 M5b 已按
fast-fail 收尾，项目定位为有验证边界的学习/研究工具。

## 角色与使用情景

| 角色 | 现在能据此做什么 | 不能据此声称什么 |
| --- | --- | --- |
| M5 研究者 | 复核时沿用先冻结、后基线的固定顺序，区分 `partial`、`unresolvable` 与工具输出 | 不能用污染或部分标注计算正式指标 |
| API/DevEx 维护者 | 理解 operation 删除为何要求明确决策 | 不能把生成资产当成人工维护者需求 |
| 生成器维护者 | 检查结构化源与生成出口是否同步 | 不能从 PCH-01 反推独立 gold |
| SDK contract-test 维护者 | 在有 Registry 权限的上游流水线检查实际重建是否删除或修复 stale test | 不能把配置声明或 C 类自动资产当作 Procedure CI 增量价值 |
| 产品投资决策者 | 用 0/12 覆盖、0 个完整 gold、0 个 A 类维护者执行停止规则 | 不能据此授权 Arazzo 1.0、OpenAPI 3.0 或 source-name 兼容开发 |

冻结后的代表性基线已经完成，结果见 [M5b 对照基线](M5-BASELINES.md)；决策 0004 已停止
产品化。本冻结记录不再提供新的执行入口。
